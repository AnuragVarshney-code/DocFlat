...
public class timeroption : MonoBehaviour
{
    public GameObject ActiveTimerbtn, timerbtn, buttonLessnMore, buttonUPnDOWN;
    ...
    void Update()
    {
        if (MoveMenuPanelup)
        {
            buttonLessnMore.transform.position = Vector3.Lerp(buttonLessnMore.transform.position, ActiveTimerbtn.transform.position, moveSpeed * Time.deltaTime);
            if (buttonLessnMore.transform.localPosition.y == tempMenuPos)
            {
                MoveMenuPanelup = false;
                buttonLessnMore.transform.position = ActiveTimerbtn.transform.position;
        ...
        {
            buttonLessnMore.transform.position = Vector3.Lerp(buttonLessnMore.transform.position, timerbtn.transform.position, moveSpeed * Time.deltaTime);
            if (buttonLessnMore.transform.localPosition.y == tempMenuPos)
            {
